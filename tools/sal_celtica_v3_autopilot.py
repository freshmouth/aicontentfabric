...
DEFAULT_CONFIG = ACCOUNT_DIR / "autopilot_v3.json"
OMNI_MASTER_PROMPT = (
    "Create ONE vertical 9:16 Spanish short-form UGC clip with native spoken audio. "
    ...
    "salt texture, lighting, framing, and phone-camera realism from the reference image. "
    "Character and voice continuity are mandatory: every visible speaking/action scene in one video "
    "must preserve the same on-camera person, same face, same age, same outfit continuity, and the same "
    "Spanish accent/speaking style. Do not switch to a different person, narrator, gender, accent, or voice. "
    "Hands-only macro/detail shots may omit the face, but must not introduce a new visible person or new voice. "
...
class SalCelticaAutopilotError(RuntimeError):
    pass
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_autopilot(config, args)
    except Exception as exc:
        print(f"Sal Celtica V3 autopilot failed: {exc}", file=sys.stderr)
        return 1
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Cloud-safe Sal Celtica V3 generation and Metricool scheduling.")
    parser.add_argument("--config", default=str(DEFAULT_CONFIG), help="Account-local autopilot config.")
    parser.add_argument("--force", action="store_true", help="Run even when today is not on the 4-day cadence.")
    parser.add_argument("--today", default="", help="Override local date for cadence checks, YYYY-MM-DD.")
    parser.add_argument("--concept-id", default="", help="Force a concept_id from autopilot_v3.json.")
...
def run_autopilot(config: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    if str(config.get("account_id") or "") != ACCOUNT_ID:
        raise SalCelticaAutopilotError(f"Autopilot config must declare account_id={ACCOUNT_ID}.")
    ...
    start_date = date.fromisoformat(str(config.get("start_date") or "2026-08-03"))
    interval_days = int(config.get("interval_days") or 4)
    due = is_due(today, start_date=start_date, interval_days=interval_days)
    cycle_index = max(0, (today - start_date).days // interval_days)
    ...
    wrapper_path = resolve_account_path(str(concept["v3_config"]))
    wrapper_config = read_json(wrapper_path)
    source_config_path = resolve_path(wrapper_path.parent, str(wrapper_config["source_config"]))
    source_config = read_json(source_config_path)

    v3_manifest = prepare_v3_manifest(wrapper_config, source_config, source_config_path, run_dir)
    first_frames = generate_first_frames(wrapper_config, v3_manifest, run_dir, config)
    ...
    video_manifest = write_video_manifest(run_dir, publish_ready, concept, publish_at)
...
def select_concept(config: dict[str, Any], *, cycle_index: int, forced_id: str) -> dict[str, Any]:
    ...
    if not concepts:
        raise SalCelticaAutopilotError("autopilot_v3.json requires at least one concept.")
    if forced_id:
        for concept in concepts:
            if str(concept.get("concept_id") or "") == forced_id:
                return dict(concept)
        raise SalCelticaAutopilotError(f"Unknown concept_id: {forced_id}")
...
def prepare_v3_manifest(
    ...
    manifest = {
        "schema_version": 3,
        "module": "sal_celtica_v3_autopilot",
...
def generate_first_frames(
    ...
    if not bool((autopilot_config.get("first_frame") or {}).get("enabled", True)):
        raise SalCelticaAutopilotError("Sal Celtica autopilot requires first_frame.enabled=true.")
    api_key = os.environ.get("OPENAI_API_KEY", "").strip()
    if not api_key:
        raise SalCelticaAutopilotError("OPENAI_API_KEY is required for first-frame generation.")
    ...
    if bool(first_frame_config.get("with_cloudinary_refs", True)):
        references = load_reference_images(wrapper_config, run_dir)
    generated: list[Path] = []
    for record in records:
        ...
        if references:
            prompt += (
                ...
                "lighting, framing, hand/object naturalness, and salt texture. Do not copy identities, logos, "
...
def load_reference_images(wrapper_config: dict[str, Any], run_dir: Path) -> list[Path]:
    ...
    try:
        ...
        if not cached_value:
            raise
        cached_dir = resolve_path(ACCOUNT_DIR, cached_value)
...
def build_run_omni_config(
    ...
    *,
    ...
    if len(records) != len(first_frames):
        raise SalCelticaAutopilotError("First-frame count does not match scene count.")
    patched = copy.deepcopy(source_config)
    patched["master_prompt"] = OMNI_MASTER_PROMPT
...
def patch_reference_images(config: dict[str, Any], first_frames: list[Path]) -> None:
    ...
    try:
        next(paths)
        raise SalCelticaAutopilotError("Unused first frames remained after patching Omni config.")
...
def resolve_variant_video(omni_dir: Path) -> Path:
    ...
    if not path.exists() or path.stat().st_size <= 0:
        raise SalCelticaAutopilotError(f"Expected Omni final video was not created: {path}")
    return path
...
def write_video_manifest(run_dir: Path, video_path: Path, concept: dict[str, Any], publish_at: datetime) -> Path:
...
def run_logged(
    ...
    *,
    ...
    if result.returncode != 0:
        raise SalCelticaAutopilotError(f"Command failed with exit code {result.returncode}. See {stderr_path}")
...
def resolve_account_path(value: str) -> Path:
    return resolve_path(ACCOUNT_DIR, value)
